		...
		<html>
			...
			<body link="#0c0000" vlink="#050000">
				<table border="1" bgcolor="#ffffff" cellspacing="0" cellpadding="0" id="CTRL1">
					...
					<tbody id="CTRL2">
						<xsl:for-each select="us_postal_codes">
							<!-- Cache the current node incase the a field is formatted -->
							<xsl:value-of select="fx:CacheCurrentNode(.)"/>
							<tr>
								<td>
									<xsl:value-of select="fx:FormatFromXSL('Zip_Code', 'General Number', '', '', 5)"/>
								</td>
								<td>
									<xsl:value-of select="fx:FormatFromXSL('Place_Name', '@', '', '', 202)"/>
								</td>
								<td>
									<xsl:value-of select="fx:FormatFromXSL('State', '@', '', '', 202)"/>
								</td>
								<td>
									<xsl:value-of select="fx:FormatFromXSL('County', '@', '', '', 202)"/>
								</td>
								<td>
									<xsl:value-of select="fx:FormatFromXSL('Lat', 'General Number', '', '', 5)"/>
								</td>
								<td>
									<xsl:value-of select="fx:FormatFromXSL('Long', 'General Number', '', '', 5)"/>
								</td>
							</tr>
						</xsl:for-each>
					</tbody>
