...
function spm_initMyMenu(oXML, oCtl)   //Creates SolpartMenu object and calls generate method
{
	...
	// Kumaran (11/11/2005) - During Agent Branding, the XML document gets skewed
	//	so we need to replace all illegal characters with XML specific charachters	
	if(oXML != null && oXML.documentElement == null) // this happens only for IE
	{
		var sXML = oXML.innerHTML.replace(/&([^(lt;)(gt;)])/gi, '&amp;$1');		
...
//  var me = this;  //allow attached events to reference this
  ...
  this.menuItemPosDivider=spm_getAttr(o, 'MenuItemPosDivider', '');
  this.menuItemLengthAdjuster=spm_getAttr(o, 'MenuItemLengthAdjuster', '');  
  ...
  this.cssMenuIconSel = spm_getAttr(o, 'CSSMenuIconSel', '');	// Chad Boettcher (01/26/2006)
  this.cssMenuArrowSel = spm_getAttr(o, 'CSSMenuArrowSel', '');	// Chad Boettcher (01/26/2006)
...
SolpartMenu.prototype.destroy = function ()
{
  ...
  this.cssMenuIconSel = null;	// Chad Boettcher (01/26/2006)
  this.cssMenuArrowSel = null;	// Chad Boettcher (01/26/2006)
...
__db(this._m_oMenu.id + ' - GenerateMenuHTML');
	...
	if (oXML == null)
	{
	  if (this._m_oDOM == null)
	  {
	    ...
	    if (this.xml.length)
	    {
			...
			if(oXML.childNodes[0].nodeName == "parsererror")
			{
				oXML = null;
				...
				this.xml = this.xml.replace(/&([^(lt;)(gt;)])/gi, '&amp;$1');		
				oXML.loadXML(this.xml);				
			}
	     }
  ...
  else
  {
      ...
      sHTML +=         this.GetMenuItems(this._m_oDOM.documentElement);
      // Kumaran (11/16/2005) - The following line adds an "Emtpy Cell" to the horizontal menu. Currently disabling it.
...
SolpartMenu.prototype.GetMenuItems = function (oParent)
{
  ...
  __db(sID + ' getmenuitems');
		  ...
		  {
	        // 02/27/2007 Chad Boettcher - Added check to see if menu is visible or not.
	        if(oNode.getAttribute("visible") == "1")
	        {
			    if (this.display == "vertical")
  			    ...
  			    else
  			    {
					    ...
					    sHTML += '<td unselectable="on" NOWRAP="NOWRAP" ' + sAlign + ' TITLE="' + spm_getAttr(oNode, 'tooltip', '') + '">' +                             spm_getItemHTML(oNode, 'left', '') +       spm_getAttr(oNode, 'title', '') + spm_getItemHTML(oNode, 'right') + MyIIf(Number(this.rootArrow) && spm_nodeHasChildren(oNode), '</td>\n<td align="right" class="' + spm_fixCSSForMac(this.getIntCSSName('spmrarw') + this.cssMenuRootArrow) + '">' + spm_getArrow(this.rootArrowImage, this) + "", '') +       '\n</td>\n</tr>\n</table>\n</td>\n';					
			    }
		  ...
		  else                        //'submenu - not top level menu item
		  {
			  switch(oNode.nodeName)
			  {
				  case "menuitem":
				  {
					  if(oNode.getAttribute("visible") == "0")
						continue;
  ...
  //--- menubar click event ---//
	SolpartMenu.prototype.onMBC = function (e, evt)
	{
		...
		if (oMenu != null)
		{
			//Uncommented - Chad J. Boettcher - 11/29/2006
  ...
  //--- handles mouseover for menu item ---//
	SolpartMenu.prototype.handlembi_mo = function (oRow, bBypassDelay)
	{
		...
		// Chad Boettcher (01/26/2006)	In order to dynamically set the background image for a 
		//								selected cell using CSS, we can't set the icon and 
		//								arrow cells because it puts the image in all of the cells
		
		// Chad Boettcher (01/26/2006) spm_getById("icon" + sID).className = spm_fixCSSForMac(this.getIntCSSName('spmitmsel spmicn') + this.cssMenuIcon + ' ' + this.cssMenuItemSel + ' ' + spm_getAttr(oRow, 'saveselcss', ''));
		spm_getById("icon" + sID).className = spm_fixCSSForMac(this.getIntCSSName('spmicnsel') + this.cssMenuIconSel);// + ' ' + spm_getAttr(oRow, 'saveselcss', ''));
		...
		// Chad Boettcher (01/26/2006) spm_getById("arrow" + sID).className = spm_fixCSSForMac(this.getIntCSSName('spmitmsel spmarw') + this.cssMenuItemSel + ' ' + this.cssMenuArrow + ' ' + spm_getAttr(oRow, 'saveselcss', ''));
		spm_getById("arrow" + sID).className = spm_fixCSSForMac(this.getIntCSSName('spmarwsel') + this.cssMenuArrowSel); // + ' ' + this.cssMenuArrow + ' ' + spm_getAttr(oRow, 'saveselcss', ''));
